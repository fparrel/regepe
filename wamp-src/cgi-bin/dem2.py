...
def GetEleFromDem(lat,lon):
    ...
    if(lon>=0.0):
        ...
        fnamelon = 'W%03d'% (-ilon)
    fname1='dem1/%s%s.zip'%(fnamelat,fnamelon)
    fname3='dem3/%s%s.zip'%(fnamelat,fnamelon)
    if data.has_key(fname1):
        ...
        if demnb==1:
            #data[fname1] = struct.unpack(unpack1,zip.read(zip.namelist()[0]))
            data[fname1] = zip.read(zip.namelist()[0])
        else:
            #data[fname3] = struct.unpack(unpack3,zip.read(zip.namelist()[0]))

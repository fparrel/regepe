...
/* Called by Dygraph when selection changed on one of the graphs */
function onGraphSelectionChanged(x1,x2) {
    refreshSelection(x1,x2);
}

function changeSelection(x1,x2) {
    refreshSelection(x1,x2);
    moveChartMarkerLeft(x1,-1);
    moveChartMarkerRight(x2,-1);
}
...
function moveChartMarkerLeft(pt1_id,chartid) {
    ...
    for (i=0;i<dygraphs.length;i++) {
        if((typeof dygraphs[i].currentSelection=='undefined')||(dygraphs[i].currentSelection==null)) {
            prevsel = [0,0,0,0,0,0];
        }
        else {
            //alert(i+" "+dygraphs[i].currentSelection);
            prevsel = dygraphs[i].currentSelection;
        }
        x1 = Math.round(dygraphs[i].layout_.points[0][pt1_id].canvasx);
        x2 = prevsel[2];
        y1 = 0;
        y2 = dygraphs[i].height_;
        dygraphs[i].currentSelection = [Dygraph.HORIZONTAL,x1,x2,y1,y2,prevsel[0],prevsel[2],prevsel[4]];
    }
    /*
    var i;
    for (i=0;i<chart.length;i++) {
...
/*function moveChartMarkerLeftOne(pt1_id,chartid) {
    if(chart[chartid].pt2px==null) {
		document.getElementById(chart[chartid].name+"chartmarkerleft").style.left = (pt1_id+chart[chartid].marginsize+21)+"px";
	}
	else {
		document.getElementById(chart[chartid].name+"chartmarkerleft").style.left = (chart[chartid].pt2px[pt1_id]+chart[chartid].marginsize+21)+"px";
	}
...
function moveChartMarkerRight(pt2_id,chartid) {
    ...
    for (i=0;i<dygraphs.length;i++) {
        ...
        x1 = prevsel[1];
        x2 = Math.round(dygraphs[i].layout_.points[0][pt2_id].canvasx);
        y1 = 0;
        y2 = dygraphs[i].height_;
        dygraphs[i].currentSelection = [Dygraph.HORIZONTAL,x1,x2,y1,y2,prevsel[0],prevsel[2],prevsel[4]];
        alert(dygraphs[i].currentSelection);
        dygraphs[i].drawZoomRect_.apply(dygraphs[i].currentSelection);
        //dygraphs[i].updateSelection_(undefined);
    }
    /*var i;
    for (i=0;i<chart.length;i++) {
...
/*function moveChartMarkerRightOne(pt2_id,chartid) {
	if(chart[chartid].pt2px==null) {
		document.getElementById(chart[chartid].name+"chartmarkerright").style.left = (pt2_id-9+chart[chartid].marginsize+21)+"px";
	}
	else {
		document.getElementById(chart[chartid].name+"chartmarkerright").style.left = (chart[chartid].pt2px[pt2_id]-9+chart[chartid].marginsize+21)+"px";
	}
	document.getElementById(chart[chartid].name+"chartmarkerright").style.display = "inline";
	chart[chartid].pt2_id = pt2_id;
}*/

/*function dygraphZoomCb(minDate, maxDate, yRanges) {
    alert('dygraphZoomCb');
}*/
...
function moveChartMarkerCurrentPoint(ptid) {
    var i;
    for (i=0;i<dygraphs.length;i++) {
        dygraphs[i].setSelection(ptid);
    }
	/*for (i in chart) {
		document.getElementById(chart[i].name+"markercurrentpoint").style.display = "inline";
		if(chart[i].pt2px==null) {
			document.getElementById(chart[i].name+"markercurrentpoint").style.left = (ptid+chart[i].marginsize+21)+"px";
		}
		else {
			document.getElementById(chart[i].name+"markercurrentpoint").style.left = (chart[i].pt2px[ptid]+chart[i].marginsize+21)+"px";
		}
	}*/
}
...
function addDemizeButton() {
    ...
    demizebtndiv.className = 'dygraph-dem';
    ...
    document.getElementById('eledistchart').style.marginBottom = '3em';
}

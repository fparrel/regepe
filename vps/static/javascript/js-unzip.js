(function (GLOBAL) {
    ...
    JSUnzip.ZipEntry = function (binaryStream) {
        ...
        this.data      = binaryStream.getNextBytesAsBuffer(this.compressedSize);
        this.databuf   = binaryStream.result2;
    }
    ...
    JSUnzip.BigEndianBinaryStream.prototype = {
        ...
        getNextBytesAsBuffer: function (steps) {
            var res = this.getByteRangeAsBuffer(this.currentByteIndex, steps);
            this.currentByteIndex += steps;
            return res;
        },
        ...
        getByteRangeAsBuffer: function (index, steps) {
            /*var result = new Array(steps);
            var max = index + steps;
            var i = index;
            while (i < max) {
                var charCode = this.getByteAt(i);
                result[i-index] = charCode;
                i++;
            }
            return result;*/
            this.result2 = new Array(steps);
            var result = "";
            var max = index + steps;
            var i = index;
            while (i < max) {
                var charCode = this.getByteAt(i);
                this.result2[i-index] = charCode;
                if(charCode!=0) {
                    result += String.fromCharCode(charCode);
                }
                // Accounting for multi-byte strings.
